...
def main(args):
    ...
    config = {
        ...
        "env_name": "OT2Env-v2", # Using the new wrapper
        "algorithm": "PPO",
    ...
    # --- Callbacks ---
    # This callback is responsible for saving model checkpoints.
    checkpoint_callback = CheckpointCallback(
        save_freq=50000, # Save a checkpoint every 50,000 steps
        save_path=f"./models/{run.id}",
        name_prefix="rl_model",
    )
    
    # Disable model saving in the WandbCallback to prevent the symlink error.
    wandb_callback = WandbCallback(
        gradient_save_freq=1000,
        model_save_path=None, # This prevents the callback from trying to save the model
        verbose=2,
    )
...
if __name__ == '__main__':
    ...
    # **MODIFIED**: Increased the default total timesteps for a more meaningful local run.
